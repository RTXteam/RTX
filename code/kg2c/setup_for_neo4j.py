...
def main():
    ...
    subprocess.check_call(["curl", "-L", f"{rtx_kg2_repo_url}/{shinc_file_name}?raw=true",
                           "-o", f"{KG2C_DIR}/{shinc_file_name}"])
    ...
    subprocess.check_call(["curl", "-L", f"{rtx_kg2_repo_url}/{mem_file_name}?raw=true",
                           "-o", f"{KG2C_DIR}/{mem_file_name}"])
    ...
    subprocess.check_call(["curl", "-L", f"{rtx_kg2_repo_url}/{setup_script_name}?raw=true",
                           "-o", f"{KG2C_DIR}/{setup_script_name}"])

    # Now run the Neo4j setup script
    logging.info(f"Running {setup_script_name}")
    os.chdir(KG2C_DIR)
    subprocess.check_call(["bash", "-x", f"{KG2C_DIR}/{setup_script_name}"])
