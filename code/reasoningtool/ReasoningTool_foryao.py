...
def expand_ncbigene_microrna(orangeboard, node):
    ...
    ## ***** YAO & ZHENG WORK ON THIS SECTION ******
    anatomy_dict = QueryBioLink.get_anatomy_for_gene(ncbi_gene_id)
    for anatomy_id in anatomy_dict.keys():
        anatomy_node = orangeboard.add_node('anatont_anatomy', anatomy_id, desc=anatomy_dict[anatomy_id])
        orangeboard.add_rel('is_expressed_in', 'BioLink', node, anatomy_node)
    ## ***** YAO WORK ON THIS SECTION ******
...
#    pathways_set_from_uniprot = QueryUniprot.uniprot_id_to_reactome_pathways(uniprot_id_str)  ## doesn't provide pathway descriptions; see if we can get away with not using it?
    ...
    if len(entrez_gene_id) > 0:
        ...
        ## protein-to-anatomy associations: ******** YAO WORK IN THIS SECTION *********
        anatomy_ids_dict = QueryBioLink.get_anatomy_for_gene_id(entrez_gene_id_str)  
        for anatomy_id in anatomy_ids_dict.keys():
            anatomy_node = orangeboard.add_node('anatont_anatomy', anatomy_id, desc = anatomy_ids_dict[anatomy_id])
            orangeboard.add_rel('is_expressed_in', 'BioLink', node, anatomy_node)
            # ******** YAO WORK IN THIS SECTION *********
...
def expand_phenont_phenotype(orangeboard, node):
    ## ****** YAO OR ZHENG:  PLEASE PUT CODE IN HERE TO EXPAND PHENOTYPE -> ANATOMY
    pass
