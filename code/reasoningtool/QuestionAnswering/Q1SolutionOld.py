...
def answerQ1(input_disease, directed=True, max_path_len=3, verbose=False,
			 use_json=False):  # I'm thinking directed true is best
	"""
	Answers Q1.
	...
	omims = Q1Utils.get_omims_connecting_to_fixed_doid(doid, directed=directed, max_path_len=max_path_len,
													   verbose=verbose)
	...
	omims = Q1Utils.refine_omims_graph_distance(omims, doid, directed=directed, max_path_len=max_path_len,
												verbose=verbose)

	# get the ones that have high probability according to a Markov chain model
	omims, paths_dict, prob_dict = Q1Utils.refine_omims_Markov_chain(omims, doid, max_path_len=max_path_len,
																	 verbose=verbose)
	...
	results_text = Q1Utils.display_results_str(doid, to_display_paths_dict, omim_to_genetic_cond, q1_doid_to_disease,
											   probs=to_display_probs_dict)
	if not use_json:
		print(results_text)
	else:
		ret_obj = Q1Utils.get_results_object_model(doid, to_display_paths_dict, omim_to_genetic_cond,
...
def main():
	...
	parser.add_argument('-d', '--directed', action="store_true", help="To treat the graph as directed or not.",
						default=True)
	...
	parser.add_argument('-a', '--all', action="store_true",
						help="Flag indicating you want to run it on all Q1 diseases",
						default=False)
	parser.add_argument('-j', '--json', action='store_true',
						help='Flag specifying that results should be printed in JSON format (to stdout)', default=False)
