...
#    @CachedMethods.register
    ...
    @staticmethod
#    @CachedMethods.register
    def query_drug_description(ligand_id):
        res = QueryPharos.send_query_get("ligands","("+ligand_id+")")
        if res is not None:
            res_json = res.json()
            #print(res_json)
            if type(res_json)==dict:
                ret_value = res_json['description']
            else:
                ret_value = None
        else:
            ret_value = None
...
if __name__ == '__main__':
    if 1 == 1:
        ...
        print(QueryPharos.query_drug_description("254599"))

    if 0 == 1:
        print(QueryPharos.query_drug_id_by_name("lovastatin"))
        print(QueryPharos.query_drug_to_targets("254599")) 
        print(QueryPharos.query_drug_name("254599"))
