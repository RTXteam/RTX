        ...
        for result in results:
            for qnode_key, node_bindings in result.node_bindings.items():
                if qnode_key not in qg.nodes:
                    response.warning(f"While recomputing qnode keys found a node binding in the results without a corresponding query node.")
                    continue
            ...
            for qedge_key, edge_bindings in result.edge_bindings.items():
                if qedge_key not in qg.nodes:
                    response.warning(f"While recomputing qedge keys found a edge binding in the results without a corresponding query edge.")
                    continue
