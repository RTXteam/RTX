    ...
    for edge_id, edge in kg.edges.items():
        ...
        if not hasattr(edge, 'attributes') or edge.attributes is None:
            log.warning(f"KG edge {edge_id} "
                        "has no attribute \'attributes\'; skipping")
            continue
...
automated reasoning system, not just ones generated by Team ARA Expander."""
    ...
    def _resultify(self, mode: str, describe: bool = False):
        ...
        nodes_bound = {node_id: copy.deepcopy(node) for node_id, node in kg.nodes.items() \
                       if hasattr(node, "qnode_keys") and node.qnode_keys}
        edges_bound = {edge_id: copy.deepcopy(edge) for edge_id, edge in kg.edges.items() \
...
def _get_qg_without_kryptonite_portions(qg: QueryGraph, log: ARAXResponse) -> QueryGraph:
    """
