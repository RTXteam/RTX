...
def _get_results_for_kg_by_qg(kg: KnowledgeGraph,              # all nodes *must* have qnode_key specified
    ...
    node_keys_for_edges_that_are_not_valid_nodes = [edge.subject for edge in cast(Iterable[Edge], kg.edges) if not
                                                   kg.nodes.get(edge.subject)] + \
                                                  [edge.object for edge in cast(Iterable[Edge], kg.edges) if not
                                                   kg.nodes.get(edge.object)]
    ...
    invalid_qnode_keys_used_by_qedges = [edge.subject for edge in cast(Iterable[QEdge], qg.edges) if not
                                        qg.nodes.get(edge.subject)] + \
                                       [edge.object for edge in cast(Iterable[QEdge], qg.edges) if not
                                        qg.nodes.get(edge.object)]
        ...
        for qedge_key in edge.qedge_keys:
            ...
            if qedge_key in query_graph.edges:
                ...
                if parallel_qedge_keys.issubset(node_pair_to_qedge_key_map[_get_edge_node_pair_key(edge)]):
                    ...
                    if edge.subject in kg_node_keys_by_qg_id[qnode_key_1] and edge.object in kg_node_keys_by_qg_id[qnode_key_2]:
                    ...
                    if edge.subject in kg_node_keys_by_qg_id[qnode_key_2] and edge.object in kg_node_keys_by_qg_id[qnode_key_1]:
...
def _find_qnode_connected_to_sub_qg(qnode_keys_to_connect_to: Set[str], qnode_keys_to_choose_from: Set[str], qg: QueryGraph) -> Tuple[str, Set[str]]:
    ...
    for qnode_key_option in qnode_keys_to_choose_from:
        ...
        all_connected_qnode_keys = {qnode_key for qedge in all_qedges_using_qnode
                                   for qnode_key in {qedge.subject, qedge.object}}.difference({qnode_key_option})
...
def _create_result_graphs(kg: KnowledgeGraph,
        ...
        if edge.qedge_keys:
            for qedge_key in edge.qedge_keys:
                ...
                if qedge_key in set(qg.edges):
                    edge_node_pair = f"{edge.subject}--{edge.object}"
                    ...
                    if ignore_edge_direction:
                        node_pair_in_other_direction = f"{edge.object}--{edge.subject}"
    ...
    for result_graph in result_graphs:
        for qedge_key in result_graph['edges']:
            ...
            potential_nodes_1 = result_graph['nodes'][qedge.subject]
            potential_nodes_2 = result_graph['nodes'][qedge.object]
