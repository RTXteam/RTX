...
automated reasoning system, not just ones generated by Team ARA Expander."""
    ...
    def _clean_up_kg(self):
        ...
        node_keys_used_in_results = {node_binding.kg_id for result in results for node_binding in result.node_bindings}
        cleaned_kg = KnowledgeGraph(nodes={node_key: node for node_key, node in kg.nodes.items() if node_key in node_keys_used_in_results},
                                    edges={edge_key: edge for edge_key, edge in kg.edges.items() if
                                           {edge.source_id, edge.target_id}.issubset(node_keys_used_in_results)})
    ...
    try:
        for edge in graph.edges:
            if droploops and edge.target_id == edge.source_id:
                continue
            if directed:
                edge_node_key = edge.source_id
                adj_map_out[edge_node_key].add(edge.target_id)
                edge_node_key = edge.target_id
                adj_map_in[edge_node_key].add(edge.source_id)
            else:
                edge_node_key = edge.source_id
                adj_map[edge_node_key].add(edge.target_id)
                edge_node_key = edge.target_id
                adj_map[edge_node_key].add(edge.source_id)
    except KeyError:
...
def _get_results_for_kg_by_qg(kg: KnowledgeGraph,              # all nodes *must* have qnode_key specified
    ...
    qnode_keys_mapped_that_are_not_in_qg = [qnode_key for qnode_key in kg_node_keys_by_qg_id if qnode_key not in qg.nodes]
    ...
    node_keys_for_edges_that_are_not_valid_nodes = [edge.source_id for edge in cast(Iterable[Edge], kg.edges) if not
                                                   kg.nodes.get(edge.source_id)] + \
                                                  [edge.target_id for edge in cast(Iterable[Edge], kg.edges) if not
                                                   kg.nodes.get(edge.target_id)]
    if len(node_keys_for_edges_that_are_not_valid_nodes) > 0:
        raise ValueError("KG has Edges that refer to the following non-existent Nodes: " + str(node_keys_for_edges_that_are_not_valid_nodes))
    ...
    for qedge_id, kg_edge_ids_for_this_qedge_id in kg_edge_ids_by_qg_id.items():
        ...
        for edge_id in kg_edge_ids_for_this_qedge_id:
            ...
            if qg_source_node_key != qg_target_node_key:
                edge_valid_in_same_direction = (kg_source_node_key in kg_node_keys_by_qg_id[qg_source_node_key] and
                                                kg_target_node_key in kg_node_keys_by_qg_id[qg_target_node_key])
                edge_valid_in_opposite_direction = (kg_source_node_key in kg_node_keys_by_qg_id[qg_target_node_key] and
                                                    kg_target_node_key in kg_node_keys_by_qg_id[qg_source_node_key])
    ...
    # kg_edges_keep_list = [edge for edge in kg.edges if not (edge.source_id in kg_node_keys_keep and
    #                                                         edge.target_id in kg_node_keys_keep)]
...
def _get_kg_node_keys_by_qg_id(knowledge_graph: KnowledgeGraph) -> Dict[str, Set[str]]:
    node_keys_by_qg_id = dict()
    for node_key, node in knowledge_graph.nodes.items():
        if node.qnode_keys:
            for qnode_key in node.qnode_keys:
                if qnode_key not in node_keys_by_qg_id:
                    node_keys_by_qg_id[qnode_key] = set()
                node_keys_by_qg_id[qnode_key].add(node_key)
    return node_keys_by_qg_id
...
def _get_kg_node_adj_map_by_qg_id(kg_node_keys_by_qg_id: Dict[str, Set[str]], knowledge_graph: KnowledgeGraph, query_graph: QueryGraph) -> Dict[str, Dict[str, Dict[str, Set[str]]]]:
    ...
    kg_node_to_node_map = {qnode_key: dict() for qnode_key in kg_node_keys_by_qg_id}
    for qnode_key in query_graph.nodes:
        node_keys_set = kg_node_keys_by_qg_id[qnode_key]
        ...
        for qedge_id in edge.qedge_ids:
            ...
            if qedge_id in all_qedge_ids:
                ...
                if parallel_qedge_ids.issubset(node_pair_to_qedge_id_map[_get_edge_node_pair_key(edge)]):
                    ...
                    if edge.source_id in kg_node_keys_by_qg_id[qnode_key_2] and edge.target_id in kg_node_keys_by_qg_id[qnode_key_1]:
...
def _get_all_adjacent_nodes(kg_node_keys: Set[str], start_qnode_key: str, target_qnode_key: str,
    ...
    """
    connections = [kg_node_adj_map_by_qg_id[start_qnode_key][kg_node_key][target_qnode_key] for kg_node_key in kg_node_keys]
        ...
        for qnode_key in fulfilled_qnode_keys:
            ...
            for corresponding_node_key in corresponding_node_keys:
                for neighbor_qnode_key in required_neighbor_qnode_keys:
                    ...
                    neighbors_in_kg = kg_node_adj_map_by_qg_id[qnode_key][corresponding_node_key][neighbor_qnode_key]
...
def _create_result_graphs(kg: KnowledgeGraph,
    ...
    result_graphs = []
    kg_node_keys_by_qg_id = _get_kg_node_keys_by_qg_id(kg)
    kg_node_adj_map_by_qg_id = _get_kg_node_adj_map_by_qg_id(kg_node_keys_by_qg_id, kg, qg)
    ...
    while qnode_keys_remaining:
        ...
        if not result_graphs:
            all_node_keys_in_kg_for_this_qnode_key = kg_node_keys_by_qg_id.get(current_qnode_key)
