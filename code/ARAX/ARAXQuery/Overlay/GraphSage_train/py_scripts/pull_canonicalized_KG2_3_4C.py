...
query = "match (n) where (((n.preferred_type<>'disease' and n.preferred_type<>'phenotypic_feature') and ('disease' in n.types or 'phenotypic_feature' in n.types)) or ((n.preferred_type<>'drug' and n.preferred_type<>'chemical_substance') and ('drug' in n.types or 'chemical_substance' in n.types))) or (('disease' in n.types and 'drug' in n.types) or ('disease' in n.types and 'chemical_substance' in n.types) or ('phenotypic_feature' in n.types and 'drug' in n.types) or ('phenotypic_feature' in n.types and 'chemical_substance' in n.types)) with COLLECT(DISTINCT n.id) as exclude_id match (m1)-[]-(m2) where m1<>m2 and not m1.id in exclude_id and not m2.id in exclude_id and not (m1.preferred_type='disease' and m2.preferred_type='drug') and not (m1.preferred_type='drug' and m2.preferred_type='disease') and not (m1.preferred_type='disease' and m2.preferred_type='chemical_substance') and not (m1.preferred_type='chemical_substance' and m2.preferred_type='disease') and not (m1.preferred_type='phenotypic_feature' and m2.preferred_type='chemical_substance') and not (m1.preferred_type='chemical_substance' and m2.preferred_type='phenotypic_feature') and not (m1.preferred_type='drug' and m2.preferred_type='phenotypic_feature') and not (m1.preferred_type='phenotypic_feature' and m2.preferred_type='drug') with distinct m1 as node1, m2 as node2 return node1.id as source, node2.id as target"
