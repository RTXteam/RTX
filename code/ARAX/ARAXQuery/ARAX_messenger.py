...
class ARAXMessenger:
    ...
    def describe_me(self):
        ...
        description_list.append(self.add_qpath(0,0,describe=True))
    ...
    ###############################################################################################
    #### Add a new QPath
    def add_qpath(self, response, input_parameters, describe=False):
        """
        Adds a new QPath object to the QueryGraph inside the Message object
        :return: ARAXResponse object with execution information
        :rtype: ARAXResponse
        """
        ...
        # #### Command definition for autogenerated documentation
        command_definition = {
            'dsl_command': 'add_qpath()',
            'description': """The `add_qpath` command adds an additional QPath to the QueryGraph in the Message object. Currently
                subject and object QNodes must already be present in the QueryGraph. The specified type is not currently checked that it is a
                valid Translator/BioLink relationship type, but it should be.""",
            'parameters': {
                'key': { 
                    ...
                    'default': '',
                    'type': 'string',
                    'description': """Any string that is unique among all QPath key fields, with recommended format e00, e01, e02, etc.
                        If no value is provided, autoincrementing values beginning for p00 are used.""",
                    },
                'subject': {
                    'is_required': True,
                    'examples': [ 'n00', 'n01' ],
        ...
        #### Scalar value filters probably come first like minimum_confidence, then complex logic filters
        #### based on edge or node properties, and then finally maximum_results
        response.info(f"Adding a QueryPath to Message with parameters {parameters}")
        ...
        if parameters['key'] is not None:
            ...
            key = self.__get_next_free_path_key()
        ...
        if parameters['subject'] is not None:
            if parameters['subject'] not in qnodes:
                response.error(f"While trying to add QPath, there is no QNode with key {parameters['subject']}", error_code="UnknownSourceKey")
                return response
            qpath.subject = parameters['subject']
        else:
            response.error(f"While trying to add QPath, subject is a required parameter", error_code="MissingSourceKey")
            return response

        #### Add the object
        if parameters['object'] is not None:
            if parameters['object'] not in qnodes:
                response.error(f"While trying to add QPath, there is no QNode with key {parameters['object']}", error_code="UnknownTargetKey")
                return response
            qpath.object = parameters['object']
        else:
    ...
    def __get_next_free_path_key(self):
        ...
        while 1:
            ...
            if index > 9:
                pad = ''
            potential_path_key = f"p{pad}{str(index)}"
            if potential_path_key not in qpaths:
