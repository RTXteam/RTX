...
class ARAXOverlay:
    ...
    def __compute_ngd(self, describe=False):
        ...
        if message and parameters and hasattr(message, 'query_graph') and hasattr(message.query_graph, 'edges'):
            allowable_parameters = {'action': {'compute_ngd'}, 'default_value': {'0', 'inf'}, 'virtual_edge_type': {
                self.parameters['virtual_edge_type'] if 'virtual_edge_type' in self.parameters else None},
            ...
            allowable_parameters = {'action': {'compute_ngd'}, 'default_value': {'0', 'inf'}, 'virtual_edge_type': {
                'any string label (optional, otherwise applied to all edges)'},
                                    'source_qnode_id': {
                                        'a specific source query node id (optional, otherwise applied to all edges)'},
                                    'target_qnode_id': {
                                        'a specific target query node id (optional, otherwise applied to all edges)'}
                                    }
...
"""
    ...
    def __predict_drug_treats_disease(self, describe=False):
        ...
        # make a list of the allowable parameters (keys), and their possible values (values). Note that the action and corresponding name will always be in the allowable parameters
        if message and parameters and hasattr(message, 'query_graph') and hasattr(message.query_graph, 'edges'):
            allowable_parameters = {'action': {'predict_drug_treats_disease'}, 'virtual_edge_type': {
                self.parameters['virtual_edge_type'] if 'virtual_edge_type' in self.parameters else None},
                                    'source_qnode_id': set([x.id for x in self.message.query_graph.nodes if
                                                            x.type == "chemical_substance"]),
                                    'target_qnode_id': set([x.id for x in self.message.query_graph.nodes if
                                                            (x.type == "disease" or x.type == "phenotypic_feature")])
                                    }
        else:
            allowable_parameters = {'action': {'predict_drug_treats_disease'}, 'virtual_edge_type': {
                'optional: any string label (otherwise applied to all drug->disease and drug->phenotypic_feature edges)'},
                                    'source_qnode_id': {
                                        'optional: a specific source query node id corresponding to a disease query node (otherwise applied to all drug->disease and drug->phenotypic_feature edges)'},
                                    'target_qnode_id': {
                                        'optional: a specific target query node id corresponding to a disease or phenotypic_feature query node (otherwise applied to all drug->disease and drug->phenotypic_feature edges)'}
                                    }
...
"""
    ...
    def __fisher_exact_test(self, describe=False):
        ...
        if describe:
            brief_description = """
`fisher_exact_test` computes the significance(p-value) of connection as node attributes to each adjacent node with certain type (e.g. 'biological_process') of the query nodes in the knowledge graph.
This information is obtained from fisher's exact test based on the contingency table with following format:
