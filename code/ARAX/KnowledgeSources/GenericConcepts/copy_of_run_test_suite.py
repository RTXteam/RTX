...
# Save every asset's status to a CSV for later review. The "Result" column is a
# plain PASS/FAIL; the "arax" column keeps the detailed status (PASSED / FAILED /
# No results / ERROR) so nothing is lost.
output_file = Path(__file__).with_name("test_suite_results_new.csv")
result_columns = ["TestCase", "TestAsset", "name", "arax", "Result", "url"]
with output_file.open("w", newline="") as fp:
    writer = csv.DictWriter(fp, fieldnames=result_columns, extrasaction="ignore")
    writer.writeheader()
    for item in final_results:
        writer.writerow({**item, "Result": "PASS" if item["arax"] == "PASSED" else "FAIL"})
