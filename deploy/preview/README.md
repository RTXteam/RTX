...
The page is a dense terminal dashboard: monospace throughout, sharp edged, no rounded cards, one
palette per theme. Each section opens with a labeled rule that runs the full width, the way a
status line divides a screen. It reads top to bottom as the live state of the box:

| section | what it is |
| --- | --- |
| header | `arax-preview` and the host name on the left, the theme switch on the right |
| host | the box facts on the left, the live metrics on the right, then the container table |
| previews | one card per pull request |
| footer | when the page was generated and how it stays current |

The earlier introduction and command reference are gone from the page. The commands work the same
way and are documented under **How to deploy** above.

The switch at the top right is a compact `[ sys ][ light ][ dark ]` segmented control. System is
the default and follows `prefers-color-scheme`, and a choice is kept in `localStorage` under
`arax-preview-theme`, per browser rather than per host. A tiny script at the top of the document
applies the stored choice before the page is painted, so a reload never flashes the other theme.
With JavaScript turned off the switch is not shown and the page follows the system.

### The host block, and the 15 second sampler

The host block is one dense panel. On the left is a column of facts about the box: the cloud and
the instance type, the region, the OS name, the docker version, the kernel and the uptime. On the
right are the live metrics, each a label, a value and a small bar: memory available of total, free
disk of total, the load averages and how many of the `PREVIEW_MAX_ACTIVE` preview slots are taken.
The memory, disk and slot metrics turn red on exactly the thresholds the preflight check refuses a
deploy on, so a red bar means the next `/deploy` gets turned away. A fact that cannot be read is
left off the list, and a metric that cannot be read shows as **no data**, so the layout never
moves. The facts come from cheap best effort collectors: the docker version out of
`docker --version`, the OS name out of `/etc/os-release`, the kernel out of `uname -r`, and the
region out of the EC2 instance metadata with a one second timeout that simply omits the region
when the box is not on EC2.

Under the panel a blinking dot and the word `live` sit next to a line that says how old the last
sample is and ticks up every second. Below that is a row per `rtx_` container with what
`docker stats` says it is using, the memory shown as usage over the limit.

The metrics and the container table are refreshed without regenerating the page. `host-stats.sh`
runs as a systemd service, samples the host every 15 seconds and writes
`${PREVIEW_WEB_ROOT}/host.json`, writing to a temporary file in the same directory and renaming it
into place so a browser can never read half a sample. The page fetches that file every 15 seconds
with a 10 second timeout, updates the metrics and the container table from it, and keeps the
sample line ticking:

```
last sample 01:08:42 UTC (7 s ago)
```

Past 60 seconds that line turns red and gains the word `stale`, the pulse dot turns red and the
metrics are dimmed, so a sampler that died is visible on the page instead of silently showing
yesterday's numbers. The bar colours come from the thresholds carried in `host.json`, which are
the same numbers the preflight check uses. With JavaScript turned off the metrics still show the
values they had when the page was generated.
...
Every collector is best effort and a number or fact that could not be read is written as `null`.
The sampler and the page generator call the same collectors in `lib.sh`, so the live metrics and
the generated page can never disagree about what free means. `host-stats.sh --once` writes a
single sample and exits, which is what the installer runs and what a test can call.
...
container. A card carries the pull request number linked to GitHub and the `RTXteam/RTX#<PR>`
path, the branch and the short commit linked to the commit, the full preview URL with the
loopback address next to it in dim, which comment started the run, when the container was created,
and whether it is running. The health dot next to the pull request is live: the page asks each
preview for its `/api/arax/v1.4/status` when it loads and turns green or red.
...
both "what is happening right now" and "did it work" without opening anything. The smoke checks
and the live queries are rendered as real tables with pass and fail glyphs, a degraded query row
shown in red, rather than as raw markdown. The full pytest output and a per-query detail file sit
behind expanders under their tables, and the raw report files are kept behind expanders too. The
spinner is a CSS animation with no image behind it.
...
| `deploy-log.txt` | `deploy.sh` | the last 200 lines of the deploy with the docker build section removed, so it is orchestration only, every line that mentions a password, token, secret or authorization dropped |
| `build-log.txt` | `deploy.sh` | the docker build section on its own, the last 400 lines, same secrets filter. Only a full rebuild writes it, and a fast redeploy deletes the one the previous deploy left |
| `smoke.md` | `smoke.sh` | the smoke test table |
| `pytest.md` | `pytest_report.sh` | the pytest summary and the failed tests, for the pull request comment |
| `pytest-full.txt` | `pytest_report.sh` | the complete captured pytest output, behind the **pytest output** expander |
| `queries.md` | `query_smoke.sh` | the live query table |
| `query-<n>.txt` | `query_smoke.sh` | one file per example query with the facts, the exact query graph that was posted, and the first 4 KB of the response body |

The deploy log and the build log no longer overlap. The build steps live once, in `build-log.txt`,
and `deploy-log.txt` is the orchestration around them with the build section cut out, so the two
expanders on the card read as two different things rather than the same build output twice.
...
It also installs the host sampler behind the live metrics on the status page. It writes one sample
...
# the sampler behind the live metrics: is it up, and what has it been saying
