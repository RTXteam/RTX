...
# Every endpoint this script handles (production, test, beta, devED, devLM, shepherd,
# complete) runs off the SYSTEM python. legacy is intentionally NOT handled here: it
# uses its own venv (/mnt/data/python/venv-legacy-arax) and a different branch, so it
# must not be redeployed with this script's master/--break-system-packages assumptions.
DEFAULT_PYTHON_BIN="/usr/bin/python3.12"
...
Options:
  ...
  -skip-pip          Skip the 'pip install -r requirements.txt' step
  -dry-run           Print what would happen, do not change anything
  -yes               Skip confirmation prompt (use with caution)
  -help              Show this help

Note: there is intentionally no 'legacy' endpoint. /legacy runs on its own branch
(last-kg2-arax-branch) and its own venv (/mnt/data/python/venv-legacy-arax); it must
not be redeployed with this script's master + --break-system-packages assumptions.
...
What it does for each endpoint:
  ...
  2) In the endpoint repo directory:
     - git stash push (TRACKED changes only; untracked files are left in place)
     - git checkout <branch> (default: master)
     - git pull --ff-only
     - git stash pop (only if a stash was created)
  3) pip install -r requirements.txt into the system python, with
     --break-system-packages (unless -skip-pip). Run once per repo.
  4) Restarts the service for that endpoint
  5) Sleeps for the configured duration
  6) Checks "service status" inside the container
  7) Checks HTTP status from the host via http://localhost (nginx -> apache -> service)
...
docker_exec_git() {
  # Run git operations as user "rt" via su - rt so we get the full login
  # environment (profile, PATH, ssh-agent, etc.).  Fall back to root if
  # the rt user does not exist inside the container.
  local cmd="$1"
  if docker exec "$CONTAINER_NAME" id -u rt >/dev/null 2>&1; then
    docker exec "$CONTAINER_NAME" su - rt -c "$cmd"
  else
    docker exec "$CONTAINER_NAME" bash -lc "$cmd"
  fi
}
...
confirm_plan() {
  ...
  echo "  pip_install: $([[ "$DO_PIP" == "1" ]] && echo "enabled ($PYTHON_BIN --break-system-packages)" || echo "disabled (-skip-pip)")"
  echo "  endpoints (current branch -> target branch):"
  for ep in "${SELECTED_ENDPOINTS[@]}"; do
    local ep_repo ep_cur ep_cur_branch ep_note
    ep_repo="$(endpoint_repo_dir "$ep")"
    # Read-only query of the endpoint's current branch+sha (does not change anything).
    ep_cur="$(git_branch_and_commit "$ep_repo" 2>/dev/null || echo "unknown ?")"
    ep_cur_branch="${ep_cur%% *}"
    if [[ "$ep_cur_branch" == "$TARGET_BRANCH" ]]; then
      ep_note="already on $TARGET_BRANCH (pull latest only)"
    else
      ep_note="WILL SWITCH BRANCH: $ep_cur_branch -> $TARGET_BRANCH"
    fi
    echo "    - endpoint: $ep"
    echo "      repo_dir: $ep_repo"
    echo "      current:  $ep_cur"
    echo "      target:   $TARGET_BRANCH   [$ep_note]"
  ...
  echo "  - git stash push (tracked changes only; only if there are local changes)"
  echo "  - git checkout $TARGET_BRANCH"
  echo "  - git pull --ff-only"
  echo "  - git stash pop (only if a stash was created)"
  if [[ "$DO_PIP" == "1" ]]; then
    echo "  - pip install -r requirements.txt ($PYTHON_BIN, --break-system-packages)"
  else
    echo "  - pip install: SKIPPED (-skip-pip)"
  fi
...
git_has_local_changes() {
  # Only TRACKED modifications count. We deliberately exclude untracked files
  # (--untracked-files=no): devareas accumulate multi-GB untracked dirs (e.g. devLM's
  # UI/interactive/{kptest,TRAPI,ARS-testing}). 'stash -u' over those is slow and hit a
  # "failed to insert into database" error during the 2026-05 migration; tracked-only
  # stash is the documented-safe path (architecture doc §9).
  local repo="$1"
  docker_exec_git "cd '$repo' && test -n \"\$(git -c safe.directory='$repo' status --porcelain --untracked-files=no)\""
}
...
pip_install_system() {
  # Install Python deps into the SYSTEM python shared by every endpoint this script
  # handles. Runs as root (writes to the system site-packages) and needs
  # --break-system-packages because the system interpreter is an externally-managed
  # (PEP 668) environment. Idempotent: pip is a no-op when requirements are satisfied.
  # NOTE: never call this for legacy — legacy must use its venv pip WITHOUT the flag.
  local repo="$1"
  if ! docker_exec_root "test -f '$repo/requirements.txt'"; then
    echo "pip_skip=1 reason=no_requirements_txt path=$repo/requirements.txt"
    return 0
  fi
  echo "action=pip_install python=$PYTHON_BIN file=$repo/requirements.txt break_system_packages=1"
  docker_exec_root "$PYTHON_BIN -m pip install --break-system-packages -r '$repo/requirements.txt'"
}
...
redeploy_endpoint() {
  ...
  # Skip git operations if this repo was already updated by a previous
  # endpoint (e.g. complete shares the production repo dir).
  if [[ -n "${UPDATED_REPOS[$repo]+x}" ]]; then
    echo "git_skip=1 reason=repo_already_updated_by_endpoint=${UPDATED_REPOS[$repo]}"
  else
    if git_has_local_changes "$repo"; then
      echo "git_local_changes=1 action=git_stash_push include_untracked=0"
      docker_exec_git "cd '$repo' && git -c safe.directory='$repo' stash push -m '$SCRIPT_NAME $(ts) endpoint=$ep' >/dev/null"
      stashed="1"
    else
    ...
    if [[ "$DO_PIP" == "1" ]]; then
      pip_install_system "$repo"
    else
      echo "pip_skip=1 reason=disabled_by_flag(-skip-pip)"
    fi

    UPDATED_REPOS["$repo"]="$ep"
  fi
  ...
  docker_exec_root "service '$service' status" 2>&1
