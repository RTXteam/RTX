...
Our approach (detailed in Sec.~\ref{sec:plan}) would advance current
...
system, based on a probabilistic graph-traversal architecture, will both learn
from repeated user interaction and also dynamically incorporate uncertainty in
all steps of the proposed process. Relevant Knowledge Sources will also be
...
ranked search strategies and answers, our Reasoning Tool will present a dossier
of results allowing the user to appreciate subtlety and complexity when it
naturally arises from an input query (e.g., some questions have more than one
...
construct the proposed Reasoning Tool within the requested 10-month time-frame.
This includes significant experience in software engineering for distributed
systems and significant research experience advancing the state-of-the-art in
systems biology
...
applications (as detailed in the previous references) and similar paradigms to
the approach that we propose to use are employed by a variety of previously
implemented knowledge engines with high-profile examples including IBM's Watson
\cite{ferrucci2010building} and the aforementioned Wolfram~Alpha
\cite{Wolframalpha}.

Finally, we propose to build the Reasoning Tool in a modular, extensible fashion
allowing it to interact with new Knowledge Sources. Since we will employ a Markov
chain architecture to generate search strategies and a Dijkstra-like algorithm
to execute them, new biological entities can easily be incorporated. The
flexibility of this framework allows for quick integration of additional
constraints and new query types with the advantage of not needing to re-train
the entire system whenever a new extension is added (in contrast to typical deep
...
we will use the widely-used Stanford parser (retrained on domain-specific data from BioNLP contests \cite{bionlp:2013})  %footnote{https://nlp.stanford.edu/software/lex-parser.shtml},
which converts the query into a directed acyclic graph 
known as the ``Stanford dependency" \cite{stanforddep:2008} (Figure~\ref{fig:parse}(a)).
For semantic parsing, we will write our own converter (see Huang biosketch) 
that transforms the Stanford dependency graph into a first-order logic formula (Figure~\ref{fig:parse}(b)).
%The NLP step will result in a first-order logic formula
%(Fig.~\ref{fig:parse}(b)).
This representation of the user query straightforwardly maps to constraints on
the Markov chain that we will use to generate a search strategy dossier (Section
\ref{section:MarkovChain}).
\begin{figure}[htbp]
\centering
\begin{tabular}{cc}
...
a {\em dossier\/} of possible {\em search strategies.\/} Here, a search
strategy is a tree for which the vertices are biological entity types (including genetic
variants, drugs, genes, molecules, pathways, diseases, diagnoses,
etc.) and the edges are query templates of Knowledge Sources to identify
...
query. To illustrate, we show the trees for three simple search strategies that
...
vertices in the tree), the Reasoning Tool would generate trees according to
...
%                    {\bf (a)} & {\bf (b)}
%  \end{tabular}
  \caption{Markov chain transition graphs for generating ranked query strategies
    ...
    generation of a branched search strategy tree.}
...
and will assign a probability to each search strategy by penalizing weak
associations and/or excessively long chains of Knowledge Source lookups.
...
The transition probabilities (i.e., the weights of the
...
for drug-variant interactions, variant-disease mechanisms, drug-disease
mechanisms, etc. We will select transition probabilities that minimize the ranks
of the 100~successful search strategies for the 100 example queries, among the
dossiers produced for these queries (using the principal axes and eigenvalues of
the Hessian to prioritize important linear combinations of the log-probability
weights to be fine-tuned in the training procedure).
%\vspace{-2ex}
...
After the search strategy (i.e., the tree generated by the Markov chain) is selected, we will
...
Dijkstra algorithm will be implemented and will search the Knowledge Sources
as specified in the search strategy.  If funded, our full Reasoning Tool will
...
biomedical entity-entity associations (Sec.~\ref{section:Dijkstra}).
...
results that are aligned with the query semantics.
