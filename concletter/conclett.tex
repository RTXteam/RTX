...
database interactions and knowledge discovery
 ...
 %                   \end{tabular}
  \caption{Three example search strategies returned by the reasoning engine: (a)
    and (b) are for a user query {\em How are disease DX and disease DY
      connected?\/}, which has two constants.  Search strategy (c) is for a
    query {\em How does variant G cause disease DX and protect against disease
      DY?}  Double-shapes denote constants in the query. Shape denotes bioentity
    type (circle: disease; hexagon: molecule; trapezoid: pathway, box,
    variant). Internal vertex labels like ``M'' and ``P'' representing the
    entity {\em type,} not a specific entity. Edge color denotes the type of
    association (blue: disease-molecule; red: molecule-pathway; green:
    disease-disease; purple: variant-molecule). Arbitrarily complex search
    ...
    graphs by the number of edges, in order to prioritize simpler search
    strategies.}
  \label{fig:ugraph}
\end{figure}
...
After the search strategy (path through the schema network) is selected, we will
...
For the POC questions, the system that we propose to build will allow the user
...
search the Knowledge Sources in real time, based on the search query.  Our
proposed system will allow for natural language input (Section
...
We would implement our proposed system in Python, using \verb|graph-tool| for
graph searching and querying, \verb|Boost.Python| for implementing the
customized weighted graph search algorithm, \verb|Py4J| for accessing Java-only
APIs as needed (see Sec.~\ref{sec:interact}), \verb|Flask| for providing a REST
API, and \verb|Requests| and \verb|json| to access the REST API for the NCATS
blackboard. We will use a Python API for Stanford CoreNLP.  We will publish all
code developed with NCATS support, on GitHub.
