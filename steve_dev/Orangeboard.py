...
class Orangeboard:
    ...
    def __del__(self):
        if hasattr(self, "driver") and not (self.driver == None):
            self.shutdown()
    ...
    def set_node_property(self, node_uuid, property_name, property_value):
        ...
        :param property_name: a ``string`` containing the name of the property (must be a valid Neo4j property name); this string parameter cannot have the value "UUID" since the Orangeboard won't allow a UUID to be changed
    ...
    def get_all_nodes(self):
        """returns a list of ``neo4j.v1.types.Node`` objects describing all nodes in the Orangeboard

        :returns: a ``list`` of ``neo4j.v1.types.Node`` objects, each describing the labels and attributes of a node
        """
        cypher_query = "MATCH (n) RETURN n"
        res = self.run_cypher_query(cypher_query)
        node_list = [ n_node[1] for record in res.records() for n_node in record.items() ]
        return node_list
    ...
    def test():
        ...
        print(all_nodes)
        node1 = all_nodes[0]
        node1_uuid = node1.properties["UUID"]
        ob.set_node_property(node1_uuid, "foo", "bar")
        print(ob.get_all_nodes()[0])
        ob.shutdown()
