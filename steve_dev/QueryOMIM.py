...
class QueryOMIM:
    ...
    def send_query_get(self, omim_handler, url_suffix):
        res = requests.get(QueryOMIM.API_BASE_URL + "/" + omim_handler + "?" + url_suffix + "&format=json",
                           cookies=self.cookie)
        assert 200==res.status_code
        return res
    
    def disease_mim_to_gene_symbols_and_uniprot_ids(self, mim_id):
        """for a given MIM ID for a genetic disease (as input), returns a dict of of gene symbols and UniProt IDs {gene_symbols: [gene_symbol_list], uniprot_ids: [uniprot_ids_list]}
        ...
        gene_symbols = [phenotype_map_list[i]["phenotypeMap"]["geneSymbols"].split(", ")[0] for i in range(0, len(phenotype_map_list))]
