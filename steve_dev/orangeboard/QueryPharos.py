...
class QueryPharos:
    ...
    def query_drug_name_to_targets(drug_name):
        # ret_ids = []
        res = QueryPharos.send_query_get("targets",
                                         "/search?q=" + drug_name + "&facet=IDG%20Development%20Level%2FTclin")
        if res is not None:
            res_json = res.json()
            # print(res_json)
            if type(res_json) == dict:
                ...
                return [{'id': content_entry['id'], 'name': content_entry['name']} for content_entry in res_content]
    ...
    def query_target_to_diseases(target_id):
        ret_ids = []
        res = QueryPharos.send_query_get("targets",
                                         "(" +target_id + ")/links(kind=ix.idg.models.Disease)")
        if res is not None:
            res_json = res.json()
            # print(res_json)
    ...
    def query_target_to_drugs(target_id):
        ret_ids = []
        res = QueryPharos.send_query_get("targets",
                                         "(" +target_id +" )/links(kind=ix.idg.models.Ligand)")
        if res is not None:
            res_json = res.json()
            # print(res_json)
            if type(res_json)==list:
    ...
    def query_drug_to_targets(drug_id):
        ret_ids = []
        res = QueryPharos.send_query_get("ligands",
                                         "(" + drug_id + ")/links(kind=ix.idg.models.Target)")
        if res is not None:
            res_json = res.json()
            # print(res_json)
            if type(res_json) == list:
